...
from .models.ModernTCN_model import (
    ModernTCNModel,
)
...
class ModernTCN:
    ...
    def detect_fit(self, train_data: pd.DataFrame, test_data: pd.DataFrame):
        ...
        config = self.config
        train_data_value, valid_data = split_train_val(train_data, 0.2)
    ...
    def detect_score(self, test: pd.DataFrame) -> np.ndarray:
        ...
        test_energy = attens_energy.reshape(-1, attens_energy.shape[-1])  # (total_B*L, F)
        return test_energy
